...
import static com.android.launcher3.LauncherPrefs.HOME_GRID_LANDSCAPE_COLUMNS;
import static com.android.launcher3.LauncherPrefs.HOME_GRID_LANDSCAPE_ROWS;
import static com.android.launcher3.LauncherPrefs.HOME_GRID_PORTRAIT_COLUMNS;
import static com.android.launcher3.LauncherPrefs.HOME_GRID_PORTRAIT_ROWS;
import static com.android.launcher3.LauncherPrefs.HOME_ICON_SIZE_PERCENT;
...
public class InvariantDeviceProfile {
    ...
    /** Bounds match the Home settings sliders and prevent malformed restored preferences. */
    private static final int HOME_GRID_MIN_COUNT = 3;
    private static final int HOME_GRID_MAX_COUNT = 10;
    private static final int HOME_ICON_SIZE_MIN_PERCENT = 70;
    private static final int HOME_ICON_SIZE_MAX_PERCENT = 130;
    ...
    InvariantDeviceProfile(
        ...
        LauncherPrefChangeListener prefListener = key -> {
            if (FIXED_LANDSCAPE_MODE.getSharedPrefKey().equals(key)
                ...
                onConfigChanged(context);
            } else if (isHomeCustomizationPreference(key)) {
                onConfigChanged(context);
            }
        };
        prefs.addListener(prefListener, FIXED_LANDSCAPE_MODE, ENABLE_TWOLINE_ALLAPPS_TOGGLE,
                HOME_GRID_PORTRAIT_COLUMNS, HOME_GRID_PORTRAIT_ROWS,
                HOME_GRID_LANDSCAPE_COLUMNS, HOME_GRID_LANDSCAPE_ROWS, HOME_ICON_SIZE_PERCENT);
        lifeCycle.addCloseable(() -> prefs.removeListener(prefListener,
                FIXED_LANDSCAPE_MODE, ENABLE_TWOLINE_ALLAPPS_TOGGLE,
                HOME_GRID_PORTRAIT_COLUMNS, HOME_GRID_PORTRAIT_ROWS,
                HOME_GRID_LANDSCAPE_COLUMNS, HOME_GRID_LANDSCAPE_ROWS,
                HOME_ICON_SIZE_PERCENT));
    ...
    private void initGrid(Context context, Info displayInfo, DisplayOption displayOption) {
        ...
        applyHomeSettingsOverrides(context, metrics, closestProfile);
    ...
    /** Returns whether the caller needs orientation-specific Home grid refreshes. */
    public boolean hasHomeGridOverrides() {
        return mPrefs.get(HOME_GRID_PORTRAIT_COLUMNS) > 0
                || mPrefs.get(HOME_GRID_PORTRAIT_ROWS) > 0
                || mPrefs.get(HOME_GRID_LANDSCAPE_COLUMNS) > 0
                || mPrefs.get(HOME_GRID_LANDSCAPE_ROWS) > 0;
    }

    private boolean isHomeCustomizationPreference(String key) {
        return HOME_GRID_PORTRAIT_COLUMNS.getSharedPrefKey().equals(key)
                || HOME_GRID_PORTRAIT_ROWS.getSharedPrefKey().equals(key)
                || HOME_GRID_LANDSCAPE_COLUMNS.getSharedPrefKey().equals(key)
                || HOME_GRID_LANDSCAPE_ROWS.getSharedPrefKey().equals(key)
                || HOME_ICON_SIZE_PERCENT.getSharedPrefKey().equals(key);
    }

    /**
     * Applies user-selected Home settings after the selected AOSP profile and partner overrides.
     * Grid overrides are orientation-specific, whereas icon sizing is intentionally consistent
     * between orientations so the visual identity of the launcher does not jump on rotation.
     */
    private void applyHomeSettingsOverrides(Context context, DisplayMetrics metrics,
            GridOption closestProfile) {
        boolean landscape = context.getResources().getConfiguration().orientation
                == Configuration.ORIENTATION_LANDSCAPE;
        int requestedColumns = landscape ? mPrefs.get(HOME_GRID_LANDSCAPE_COLUMNS)
                : mPrefs.get(HOME_GRID_PORTRAIT_COLUMNS);
        int requestedRows = landscape ? mPrefs.get(HOME_GRID_LANDSCAPE_ROWS)
                : mPrefs.get(HOME_GRID_PORTRAIT_ROWS);

        boolean hasGridOverride = requestedColumns > 0 || requestedRows > 0;
        if (hasGridOverride) {
            numColumns = sanitizeHomeGridCount(requestedColumns, numColumns);
            numRows = sanitizeHomeGridCount(requestedRows, numRows);

            // Keep all Home containers aligned with the user-selected workspace width. The folder
            // and hotseat values are bounded so custom settings remain valid on small displays.
            numSearchContainerColumns = Math.min(numSearchContainerColumns, numColumns);
            numShownHotseatIcons = Math.min(numColumns, closestProfile.numHotseatIcons);
            numDatabaseHotseatIcons = deviceType == TYPE_MULTI_DISPLAY
                    ? Math.max(numShownHotseatIcons, numColumns * 2) : numShownHotseatIcons;
            numAllAppsColumns = numColumns;
            numDatabaseAllAppsColumns = deviceType == TYPE_MULTI_DISPLAY
                    ? Math.max(numAllAppsColumns, numColumns * 2) : numAllAppsColumns;
            numAllAppsRowsForCellHeightCalculation = numRows;
            for (int i = 0; i < COUNT_SIZES; i++) {
                numFolderRows[i] = Math.min(numFolderRows[i], numRows);
                numFolderColumns[i] = Math.min(numFolderColumns[i], numColumns);
            }

            // A unique DB per orientation/grid prevents out-of-range items from disappearing
            // after rotation. The standard launcher migration path copies and reflows the active
            // workspace into this database the first time the setting is selected.
            dbFile = String.format("launcher_home_%s_%dx%d.db",
                    landscape ? "landscape" : "portrait", numColumns, numRows);
        }

        int iconSizePercent = Math.max(HOME_ICON_SIZE_MIN_PERCENT,
                Math.min(HOME_ICON_SIZE_MAX_PERCENT, mPrefs.get(HOME_ICON_SIZE_PERCENT)));
        if (iconSizePercent != 100) {
            float iconScale = iconSizePercent / 100f;
            for (int i = 0; i < COUNT_SIZES; i++) {
                iconSize[i] *= iconScale;
                allAppsIconSize[i] *= iconScale;
                transientTaskbarIconSize[i] *= iconScale;
            }
        }
    }

    private static int sanitizeHomeGridCount(int requestedValue, int fallback) {
        if (requestedValue <= 0) {
            return fallback;
        }
        return Math.max(HOME_GRID_MIN_COUNT, Math.min(HOME_GRID_MAX_COUNT, requestedValue));
    }
